...
- **Advanced grids** (Data Grid, Tree Grid, Hierarchical Grid, Pivot Grid) → `igniteui-webcomponents-grids` _(commercial)_
- **Lightweight grid** (Grid Lite) → `igniteui-grid-lite` _(MIT)_
- **Docking layout** → `igniteui-dockmanager` _(commercial)_
- **Charts & visualizations** (Category Chart, Data Chart, Financial Chart, Pie Chart, Sparkline, Treemap, etc.) → `igniteui-webcomponents-charts` _(commercial)_
